...
cols <- c(rgb(0,0,0.8,0.3), rgb(0.8, 0, 0, 0.3))
cols_dark <- c(rgb(0,0,0.8,0.9), rgb(0.8, 0, 0, 0.9))


# Mean and CI of staphylococcus aureus log CFU

plot(0, type = "n", main=expression( "Dynamics of " *italic(S.aureus)* " CFU in time"), xlim = range(d$days), xlab = "Days after decolonization",  ylim = range(d$staphCFU), ylab=expression( "Log 10 of " *italic(S.~aureus)* " CFU in time"))

for(patient_current in levels(d$patient)) {
  dsub <- d[patient == patient_current]
  lines(dsub$days, dsub$staphCFU, col = cols[dsub$carrier[1] + 1], lty = 2)
}
...
cfumed <- d[, .(cfu_mean = mean(staphCFU), cfu_cilo = mean(staphCFU) - 1.96*sd(staphCFU)/sqrt(8), cfu_cihi = mean(staphCFU) + 1.96*sd(staphCFU)/sqrt(8),
                cfu_median = median(staphCFU), cfu_q25 = quantile(staphCFU, 0.25), cfu_q75 = quantile(staphCFU, 0.75)), by = .(days, carrier)  ]

cfumed[cfu_cilo < 0, cfu_cilo := 0]

x <- cfumed[carrier == TRUE]
lines(x$days, x$cfu_mean, col = cols_dark[2], lwd=2)
polygon(c(x$days, rev(x$days), x$days[1]), c(x$cfu_cilo, rev(x$cfu_cihi), x$cfu_cilo[1]), border = NA, col = cols[2])
...
lines(x$days, x$cfu_mean, col = cols_dark[1], lwd=2)
polygon(c(x$days, rev(x$days), x$days[1]), c(x$cfu_cilo, rev(x$cfu_cihi), x$cfu_cilo[1]), border = NA, col = cols[1])
...
df <- select(d, c(carrier, days, patient, SA_prop))
...
plot(0, type = "n", main=expression( "Dynamics of " *italic(S.aureus)* " reads abundance in time"), xlim = range(df2$days), xlab = "Days after decolonization", ylim = c(0, 1.1), ylab =expression("" *italic(S.~aureus)* " reads proportion"))
legend("topleft", bty = "n", title = "", fill = cols, legend = c("Non-carrier", "Carrier"))

for(patient_current in levels(d$patient)) {
  dsub <- d[patient == patient_current]
  lines(dsub$days, dsub$SA_prop, col = cols[dsub$carrier[1] + 1], lty = 2)
}

x <- df2[carrier == TRUE]
lines(x$days, x$SAprop_mean, col = cols_dark[2], lwd=2)
polygon(c(x$days, rev(x$days), x$days[1]), c(x$SAprop_lower, rev(x$SAprop_upper), x$SAprop_lower[1]), border = NA, col = cols[2])

x <- df2[carrier == FALSE]
lines(x$days, x$SAprop_mean, col = cols_dark[1], lwd=2)
polygon(c(x$days, rev(x$days), x$days[1]), c(x$SAprop_lower, rev(x$SAprop_upper), x$SAprop_lower[1]), border = NA, col = cols[1])
